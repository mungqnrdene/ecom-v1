...
@section('page')
    ...
    <div class="dashboard-header">
        ...
        <div class="dashboard-search">
            <form action="{{ route('users.welcome') }}" method="GET" id="dashboardSearchForm">
                <input type="text" name="q" value="{{ $searchQuery ?? '' }}" placeholder="🔍 Бараа хайх..."
                    autocomplete="off">
                <button type="submit">
    ...
    <div class="section-header">
        @if ($isSearching)
            <h5 class="section-title">
                <i class="bi bi-search me-2"></i>Хайлтын үр дүн: "{{ $searchQuery }}"
            </h5>
            <span class="result-count">{{ $products->count() }} бараа олдсон</span>
        @else
            <h5 class="section-title">
                <i class="bi bi-stars me-2"></i>Шинэ бүтээгдэхүүн
            </h5>
            <span class="result-count">{{ $products->count() }} бараа</span>
        @endif
    </div>
    ...
    @if ($products->isEmpty())
        <div class="empty-state">
            <i class="bi bi-search"></i>
            <h5>"{{ $searchQuery }}" гэсэн хайлтаар бараа олдсонгүй</h5>
            <p class="text-muted">Өөр түлхүүр үгээр дахин оролдоно уу</p>
            <a href="{{ route('users.welcome') }}" class="btn btn-primary">
